...
public class ResolvedEndpoint implements ServiceEndpoint {
    ...
    public static ServiceEndpoint resolveEndpoint(DocumentFragment epr, QName elementName, QName serviceName, String uriPrefix) {
        ...
        for (Node child = epr.getFirstChild(); child != null; child = child
                .getNextSibling()) {
            if (child instanceof Element) {
                ...
                if (elementCount == 1) {
                    ...
                    if (elementName.getNamespaceURI().equals(nsUri)
                            && elementName.getLocalPart().equals(name)) {
                        resolvedEndpoint = new ResolvedEndpoint(epr, DOMUtil
                                .getElementText(elem), serviceName);
                        // Check WSA endpoints
                    } else {
                        NodeList nl = elem.getElementsByTagNameNS(
