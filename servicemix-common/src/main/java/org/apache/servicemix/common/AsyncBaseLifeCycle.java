...
public class AsyncBaseLifeCycle implements ComponentLifeCycle {
	...
	public void init(ComponentContext context) throws JBIException {
		try {
			...
			try {
				this.transactionManager = (TransactionManager) context
	...
	protected WorkManager findWorkManager() {
		...
		try {
			...
			Method getWorkManagerMth = container.getClass().getMethod(
			...
			if (logger.isDebugEnabled()) {
				logger
						.debug(
								"JBI container is not ServiceMix. Will create our own WorkManager",
								t);
			}
		}
				...
				if (tx != null) {
					...
					if (status != Status.STATUS_NO_TRANSACTION) {
						...
						try {
							...
							logger.error(
									"Error trying to rollback transaction.", t);
						}
					}
				}
			} catch (Throwable t) {
	...
	protected void processExchange(MessageExchange exchange) throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("Received exchange: status: "
					+ exchange.getStatus()
					+ ", role: "
					+ (exchange.getRole() == Role.CONSUMER ? "consumer"
							: "provider"));
		}
		if (exchange.getRole() == Role.PROVIDER) {
			...
			Endpoint ep = (Endpoint) this.component.getRegistry().getEndpoint(
					key);
			if (ep == null) {
			...
			if (processor == null) {
				throw new IllegalStateException(
						"No processor found for endpoint: " + key);
			}
			try {
			...
			if (exchange.getProperty(JbiConstants.SENDER_ENDPOINT) != null) {
				String key = exchange.getProperty(JbiConstants.SENDER_ENDPOINT)
						.toString();
				...
				processor = (ExchangeProcessor) processors.remove(exchange
						.getExchangeId());
			}
			if (processor == null) {
